...
export async function insertDataFor5K(data: Data[]) {
  ...
  console.log("SQL: start to insert 5K pokemons")
  let table = `INSERT INTO pokemons (id, name, img, height, weight, favorite) VALUES `
  let values = ""
  for (let i = 0; i < newDatabase.length; i++) {
    values += `('${newDatabase[i].id}','${newDatabase[i].name}','${newDatabase[i].img}','${newDatabase[i].height}','${newDatabase[i].weight}','${newDatabase[i].favorite}'),`
  }
  let sql = table + values.slice(0, -1) + ";"
  await client.query(sql)
  console.log("SQL: finished to insert 5K pokemons")
}
